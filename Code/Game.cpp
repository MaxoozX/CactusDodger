...
Game::Game(int windowWidth, int windowHeight, int frameRate, float constant_g, int numberOfScoreDigits, int barriersSpeed) :
              window(NULL),
              renderer(NULL),
              playerTexture(NULL),
              cactusTexture(NULL),
              ...
              playerColor(0, 255, 0),
              m_player(windowWidth / 2, windowHeight - 50 - FLOOR_HEIGHT, 100, 100, frameRate, constant_g),
              background(windowWidth / 2, windowHeight / 2, windowWidth, windowHeight),
              gameWindowWidth(windowWidth),
              gameWindowHeight(windowHeight),
              gameFrameRate(frameRate),
              scoreDigitsNumber(numberOfScoreDigits),
              waitTime(1000 / frameRate),
              gameBarriersSpeed(barriersSpeed)
{
}
...
void Game::shiftFunction() {
  shiftPressed = false;
  m_player.jump(-15000); // Go down, like a negative jump
}
...
void Game::drawNumberOnScreen(int number)
{
  ...
  for (int i(padding); i < scoreDigitsNumber; i++)
...
{
  if (lastBarrierSpawned >= 2 * gameFrameRate)
  { // Seems alright
    if (rand() % 20 < 1)
    {
